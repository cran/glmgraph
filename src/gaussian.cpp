...
RcppExport SEXP cdfit_gaussian(SEXP X_, SEXP y_, SEXP L_, SEXP diagL_, SEXP penalty_, SEXP lambda1_, SEXP lambda2_,SEXP gamma_, SEXP multiplier_, SEXP eps_, SEXP max_iter_, SEXP dfmax_,SEXP standardizeX_,SEXP warn_) {

	arma::mat X= Rcpp::as<arma::mat>(X_);
	arma::vec Y= Rcpp::as<arma::vec>(y_);
	arma::mat L= Rcpp::as<arma::mat>(L_);
	arma::mat diagL=Rcpp::as<arma::vec>(diagL_);
	...
	bool standardizeX=Rcpp::as<bool>(standardizeX_);
	    ...
	    for (i=0; i<(int)lambda1s(j).n_elem; i++) {
	        ...
	        while (TRUE) {			
	            ...
	            while (TRUE) {	
	            	...
	            	if(penalty=="lasso")  obj1=cycle_gaussian_lasso (Y, X, L, diagL, b_est1, active_set1, multiplier, _lambda1, _lambda2, standardizeX,0);
	    			else if(penalty=="MCP")  obj1=cycle_gaussian_MCP (Y, X, L, diagL, b_est1, active_set1, multiplier, _lambda1, _lambda2, gamma, standardizeX, 0);
	...
	Rcpp::List res=Rcpp::List::create(b_est_all,b_iter_all);
